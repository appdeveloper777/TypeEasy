...
| **Globales de proceso** | 165 `g_*` (lexer/parser incluidos) | **5 justificados** (`g_vm_main_storage`, 2 flags de signal handler, la tabla computed-goto del bytecode y el `SSL_CTX` compartido, en `scripts/core_debt_allow.txt`); todo lo demás vive en `TeVM`, `te_vm_cur` es `__thread`; flex/bison reentrantes (`TeParseCtx`) | `audit_core_debt.sh` (allowlist; también detecta **punteros `static` dentro de funciones** — así ASan encontró el cache `this` que apuntaba a otra VM) + `--selftest-vm` corre 2 VMs **en 2 hilos** (bajo ASan en CI) |
