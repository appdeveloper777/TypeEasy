...
| 2 | **Partir `ast.c`** por dominio, movimiento puro sin cambio semántico (un módulo por PR): `te_interp_call.c` (despacho por receptor), `te_interp_decl.c`, `te_interp_flow.c`, `te_print.c`, `te_frames.c`. Orden: de menos a más globales tocados. | ✅ **completa** (0.1.0): `te_print.c`, `te_interp_flow.c`, `te_interp_decl.c`, `te_value.c`, `te_stdlib.c` (despacho de builtins por categoría), `ast_internal.h`; `interpret_call_method_impl` 1.119→289 (`te_cm_*`), `evaluate_expression` 576→151 (`te_ev_*`), `interpret_node` (`te_stmt_*`), `request_handler` 252; 400 líneas muertas eliminadas. **0 funciones > 300 líneas** en el núcleo (`audit_core_debt`, baseline solo achica). Herramientas de extracción reproducibles en `tools/refactor/`. |
| 3 | **Estado explícito `TeVM*`**: agrupar los globales en una struct, luego pasarla como parámetro por módulo → intérprete reentrante, tests unitarios en C, base para hilos. | ✅ **A + B** (0.1.0): `te_vm.h` agrupa **todo** el estado por-VM (vars/flags/callstack/recovery/stdout/símbolos/frames; 75→58 globales, los que quedan son configuración de proceso, pools y caches). `g_vm` es una macro sobre `*te_vm_cur`: `te_vm.c` (`te_vm_create/destroy/set_current`) permite **varias VMs por proceso**, verificado por `typeeasy --selftest-vm` en CI (ASan). Los módulos extraídos reciben `TeVM *vm`. 🔜 **3C** (post 0.1.0, no bloqueante): clases/interning/lexer-parser por VM y `te_vm_cur` thread-local → hilos en vez de prefork. |
| 4 | Derivados: prepared statements MySQL (`mysql_stmt_*`) y Postgres (`PQexecParams`) en `db_params.c`; reemplazar `setjmp/longjmp` por propagación de error; unificar AST-walker y bytecode; tipo `decimal`. | ✅ **cerrada en alcance 0.1.0**: (a) `db_substitute_params` = *emulated prepared statements* con escape **del servidor** (`mysql_real_escape_string` / `PQescapeStringConn`, charset-aware) verificado por tests de inyección (`tests/db` 11/11, `12_db/sqlite_params_injection.te`); `@param` int64 sin truncar. (b) `longjmp`: el punto de recuperación es por-VM (`vm->runtime_recovery`) y la recuperación restaura **todo** el estado (frames, callstack, depth, flags de flujo, vars, conexiones) — test `tests/regress/run_api_fatal_recovery.sh` en CI. (c) Guardia `-Werror=implicit-function-declaration` (evita truncamiento de punteros LLP64). 🔜 **Post 0.1.0** (features, no deuda): server-side prepares `mysql_stmt_*`/`PQexecParams` (requieren banco de pruebas MySQL/PG en CI), unificar walker+bytecode, `decimal`. |
