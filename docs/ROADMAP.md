...
| 2 | **Partir `ast.c`** por dominio, movimiento puro sin cambio semántico (un módulo por PR): `te_interp_call.c` (despacho por receptor), `te_interp_decl.c`, `te_interp_flow.c`, `te_print.c`, `te_frames.c`. Orden: de menos a más globales tocados. | ✅ **cuts 1–3** (0.1.0): `te_print.c`, `te_interp_flow.c`, `te_interp_decl.c`, `te_value.c`, `ast_internal.h`; `interpret_call_method_impl` 1.119→687 (`te_cm_*`); 400 líneas muertas eliminadas. **`ast.c` 12.061→9.883**. Quedan > 300: `evaluate_expression` 576, `impl` 687, `interpret_var_decl` 496, `interpret_assign` 336, `interpret_println` 349 (a partir en 0.1.x). |
| 3 | **Estado explícito `TeVM*`**: agrupar los globales en una struct, luego pasarla como parámetro por módulo → intérprete reentrante, tests unitarios en C, base para hilos. | ✅ **A + B (módulos)** (0.1.0): `te_vm.h` + `TeVM g_vm` (7 estados centrales, 466 sitios, 14 `extern` eliminados); los módulos extraídos (`te_interp_flow.c`, `te_interp_decl.c`) reciben `TeVM *vm` y no tocan el global. 🔜 B completo: `vm` en `ast.c`/`te_*.c` restantes → varias VMs por proceso. |
| 4 | Derivados: prepared statements MySQL (`mysql_stmt_*`) y Postgres (`PQexecParams`) en `db_params.c`; reemplazar `setjmp/longjmp` por propagación de error; unificar AST-walker y bytecode; tipo `decimal`. | ✅ parcial (0.1.0): tests de inyección (`tests/db` 11/11, `12_db/sqlite_params_injection.te`) confirman `db_substitute_params` = *emulated prepared statements*; `@param` int64 sin truncar. 🔜 server-side prepares (mejora), `PQexecParams`, `longjmp`→error (requiere 3B completo). |
